...
public class DocumentLoader
{
    ...
    public static Collection<Document> getDocumentSet(DBConnector conn, int crossVal)
    {
        ...
        try{
            ...
            while(rs.next()){
                ...
                c.isScene = rs.getBoolean("is_scene");
                if(assocBoxIDs != null){
                      ...
                      int batchSize, int numThreads) throws Exception
    {
        ...
        query = "CREATE TABLE IF NOT EXISTS chain (img_id VARCHAR(15), "+
                ...
                "is_scene TINYINT(1), PRIMARY KEY(img_id, "+
                "chain_id));";
        conn.createTable(query);
        ...
        for(Document d : docSet){
            for(Chain c : d.getChainSet()){
                ...
                paramSet.add(new Object[]{d.getID(), c.getID(), boxIdStr, c.isScene});
            }
        }
        conn.update(query, paramSet, batchSize, numThreads);
    ...
    /**Creates and populates a dependency table in the database specified by the
     * given conn with dependency trees generated by the Stanford Dependency Parser
     * on the captions in the given docSet
     *
     * @param conn
     * @param docSet
     * @param batchSize
     ...
     * @throws Exception
     */
    public static void populateDependencyTable(DBConnector conn, Collection<Document> docSet,
                                               int batchSize, int numThreads) throws Exception
    {
        String query;
        Set<Object[]> paramSet;
